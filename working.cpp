...
// the neighbors' proc_rank can be determined by the offset by doing offset % nb_procs
std::vector<int> get_target_neighbors_proc_rank(int offset, int proc_rank, int nb_procs, int m, int n) {
	std::vector<int> neighbors(11);
	int nb_valid_neighbors = 5;
	...
	// identify neighbors
	neighbors[0] = offset - m;
	neighbors[1] = ((neighbors[0] % nb_procs) + neighbor_delta) % nb_procs;
	neighbors[2] = offset - 1;
	neighbors[3] = ((neighbors[2] % nb_procs) + neighbor_delta) % nb_procs;
	neighbors[4] = offset + 1;
	neighbors[5] = ((neighbors[4] % nb_procs) + neighbor_delta) % nb_procs;
	neighbors[6] = offset + m;
	neighbors[7] = ((neighbors[6] % nb_procs) + neighbor_delta) % nb_procs;
	neighbors[8] = offset;
	neighbors[9] = ((neighbors[8] % nb_procs) + neighbor_delta) % nb_procs;
	...
	neighbors[10] = nb_valid_neighbors;
/*	if (proc_rank == 12){
		printf("Valid neighbors: %d for ij %d, %d of proc# %d of offset %d \n", nb_valid_neighbors, i, j, proc_rank, offset);
	}*/
	return neighbors;
